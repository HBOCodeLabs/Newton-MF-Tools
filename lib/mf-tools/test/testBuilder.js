...
(function() {
  ...
  TestBuilder = (function() {
    function TestBuilder() {
      ...
      this._mochaCommand = function(alltests) {
        return 'mocha';
      };
    }
    ...
    TestBuilder.prototype.mochaCommand = function(_mochaCommand) {
      this._mochaCommand = _mochaCommand;
      return this;
    };
    ...
    TestBuilder.prototype.build = function() {
      ...
      this._task("test", "Run all tests", function() {
        ...
        if (_this._runTogether) {
          return _this._runTests(_this._getAllTestFiles(), 'Starting all tests...', true);
        } else {
          _results = [];
    ...
    TestBuilder.prototype._testCmd = function(alltests) {
      return "NODE_PATH=$NODE_PATH:" + (this._includePaths.join(':')) + " " + (this._mochaCommand(alltests)) + " --globals " + (this._globals.join(',')) + " -u " + this._mochaUi + " -R " + this._mochaReporter + " --require " + (this._mochaPreRequire());
    };

    TestBuilder.prototype._runTests = function(files, msg, alltests) {
      var file, _i, _len;

      if (this._runTogether) {
        this._runTestProcess(files, msg, alltests);
      } else {
    ...
    TestBuilder.prototype._runTestProcess = function(files, msg, alltests) {
      ...
      console.log('testCmd: ', this._testCmd(alltests));
      child = exec("" + (this._testCmd(alltests)) + " " + (files.join(' ')), {
        env: childEnv,
        encodig: 'utf8'
